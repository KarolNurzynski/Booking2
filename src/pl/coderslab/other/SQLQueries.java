...
public class SQLQueries {
	...
	String reservation = "create table reservation (id int(11) not null auto_increment, order_date date not null, checkin_date date not null, checkout_date date not null, description text, user_id int(11) not null, room_id int(11) not null, status_id int(1) not null, primary key(id), foreign key(user_id) references user(id), foreign key(room_id) references room(id), foreign key(status_id) references status(id)) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
	String status = "create table status (id int(1) not null auto_increment, name varchar(20) not null, primary key(id)) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
